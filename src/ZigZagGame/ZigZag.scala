...
object ZigZag {
  ...
  def _play(board: Board, word: List[Char], coords: List[Coord2D], dirs: List[Direction]): Boolean = dirs match {
    case Nil => false
  ...
  def _getChar(board: Board, coord: Coord2D, acc: Int): Char = board match {
    case Nil => Empty
